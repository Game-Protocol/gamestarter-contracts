...
contract('BancorConverter', accounts => {
  ...
  it('verifies the manager can update the fee', async () => {
    ...
    await converter.acceptManagement({
      from: accounts[4]
    });
  ...
  it('should throw when a non owner attempts to update a connector', async () => {
    ...
    try {
      await converter.updateConnector(connectorTokenAddress, 200000, false, 0, {
        from: accounts[1]
      });
      assert(false, "didn't throw");
    } catch (error) {
  ...
  it('should throw when a non owner attempts to withdraw from the connector', async () => {
    ...
    try {
      await converter.withdrawTokens(connectorToken.address, accounts[3], 50, {
        from: accounts[1]
      });
      assert(false, "didn't throw");
    } catch (error) {
